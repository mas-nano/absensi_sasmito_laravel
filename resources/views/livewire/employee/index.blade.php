...
<x-page-layout>
    <x-slot:breadcrumbs>
        ...
        <li class="flex items-center space-x-1">
            ...
            <span class="px-1 py-2 text-black dark:text-white text-sm">Pegawai</span>
        </li>
    </x-slot:breadcrumbs>
    <div class="flex justify-between items-center">
        <p class="py-1 px-2 text-sm font-semibold text-black dark:text-white">Pegawai</p>
        <div class="">
            <button class="py-1 px-2 bg-black dark:bg-[#a4a5f7] text-xs text-white dark:text-black rounded-lg"
                wire:click="$dispatch('openModal', { component: 'employee.import'})">Import Pegawai</button>
            ...
            @forelse ($employees as $item)
                <div class="p-6 rounded-2xl dark:bg-[#FFFFFF0D] bg-[#E5ECF680] grid grid-cols-5 gap-4"
                    ...
                    <div class="col-span-2">
                        <img src="{{ $item->profile_picture != null ? asset('storage/' . $item->profile_picture) : asset('assets/img/no_profile.jpeg') }}"
                            class="w-full object-cover object-center h-full" alt="">
                    </div>
                    <div class="col-span-3 flex flex-col">
                        <div class="flex-1">
                            <p class="text-2xl font-semibold text-black dark:text-white">{{ $item->first_title }}
                                {{ $item->name }}
                                {{ $item->last_title }}</p>
                            <p class="text-black dark:text-white text-sm">{{ $item->phone_number }}</p>
                            <p class="text-black dark:text-white text-sm">{{ $item->address }}</p>
                        </div>
                        <div class="flex justify-end space-x-3">
                            <a href="{{ route('employee.show', $item->uuid) }}" wire:navigate><i
                                    class="ph-duotone ph-eye text-blue-500 text-lg"></i></a>
                            <a href="{{ route('employee.edit', $item->uuid) }}" wire:navigate><i
                                    class="ph-duotone ph-pencil-simple text-yellow-500 text-lg"></i></a>
                            <button type="button"
                                wire:click="$dispatch('openModal', { component: 'employee.delete-modal', arguments:{id:'{{ $item->user->id }}'} })"><i

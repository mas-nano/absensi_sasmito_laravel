...
class ProjectController extends Controller
{
    ...
    public function getUsers(Project $project, Request $request): JsonResponse
    {
        return $this->responseSuccessWithData('user data', ['users' => User::with(['profile', 'role', 'attendances' => function ($query) use ($request) {
            $query->whereBetween('created_at', [Carbon::parse($request->query('date', date('Y-m-d'))), Carbon::parse($request->query('date', date('Y-m-d')))->addDay()]);
    ...
    public function listAttendance(Project $project, Request $request): JsonResponse
    {
        ...
        $user = User::with(['profile', 'attendances' => function ($query) use ($project, $validated) {
            $query->where('project_id', $project->id)->whereBetween('created_at', [Carbon::parse($validated['date']), Carbon::parse($validated['date'])->addDay()]);
